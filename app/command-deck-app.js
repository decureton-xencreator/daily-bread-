...
document.addEventListener('click',event=>{
  ...
  if(event.target.closest('[data-activation-retry]')){render();recordInteraction('activation','retry-evaluation');const gate=activationCenterModel().currentGate;notify(`Evaluation refreshed. ${gate} remains the truthful current gate.`)}
  ...
  if(event.target.closest('[data-lesson-submit]')&&activeLesson){
    ...
    const submission=submitAcademyResponse(state.academy,activeLesson,response);state.academy=submission.academy;saveState();recordInteraction('academy',submission.result.passed?'answer-pass':'answer-retry');if(!auditRecoveryResult(submission.result).passed)recordFinding('recovery-contract-failed',activeLesson);renderLesson();
  }
  if(event.target.closest('[data-lesson-retry]')&&activeLesson){state.academy=retryAcademyActivity(state.academy,activeLesson);saveState();recordInteraction('academy','correction-started');renderLesson()}
  if(event.target.closest('[data-lesson-hint]')){const hint=$('[data-coach-hint]'),button=event.target.closest('[data-lesson-hint]');hint.hidden=!hint.hidden;button.setAttribute('aria-expanded',String(!hint.hidden));recordInteraction('academy','coaching-hint')}
  if(event.target.closest('[data-lesson-defer]')&&activeLesson){const deferred=deferAcademyActivity(state.academy,activeLesson);state.academy=deferred.academy;saveState();if(deferred.deferred){recordInteraction('academy','answer-deferred');renderLesson()}else notify('This is the only unfinished activity. Correct it now or pause and resume later.')}
