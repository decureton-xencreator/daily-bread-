...
required += ['assets/css/command-deck.css','assets/images/earth-blue-marble-nasa.jpg','app/command-deck-app.js','tests/command-deck.test.mjs','releases/XPS-4.0.0.json','releases/XPS-4.1.0.json','governance/sync/XDBS-COMMAND-DECK-2026-07-24.md']
...
required += ['app/voice-runtime.js','app/voice-ui.js','assets/css/voice-assessment.css','tests/voice-runtime.test.mjs','releases/XPS-4.5.0.json','governance/sync/XDBS-VOICE-ASSESSMENT-2026-07-25.md']
for p in required:
...
for marker in ['COMMAND DECK · DAILY BREAD','deck-progress','deck-count','scene-index-list','assets/images/earth-blue-marble-nasa.jpg']:
 if marker not in index:issues.append(f'missing-ui-marker:{marker}')
...
report={'schemaVersion':'2.6','validatedAt':'2026-07-24T05:01:00-04:00','result':'PASS' if not issues else 'FAIL','checks':{'requiredFiles':not any(x.startswith('missing:') for x in issues),'jsonParse':not any(x.startswith('invalid-json') for x in issues),'internalLinks':not any(x.startswith('broken-link') for x in issues),'safeExternalLinks':not any(x.startswith('unsafe-external-link') for x in issues),'controls':not any(x.startswith('empty-button') for x in issues),'responsiveViewport':not any(x.startswith('viewport') for x in issues),'reducedMotion':'reduced-motion:missing' not in issues,'archiveIntegrity':not any(x.startswith(('archive-missing','current-alias','current-release')) for x in issues),'academyPublicMetrics':'public-academy-metrics' not in issues,'secretScan':not any(x.startswith('possible-secret') for x in issues),'deploymentEvidence':deployment_verified},'issues':issues,'warnings':warnings}
...
print('XDBS 2.6 validation passed')
